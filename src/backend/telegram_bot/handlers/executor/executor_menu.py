...
@check_state(required_state="executor_idle")
async def handle_executor_confirm_complete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Обрабатывает подтверждение завершения заказа исполнителем.
    """
    query = update.callback_query
    callback_data = query.data
    order_id = int(callback_data.split("_")[-1])
    ...
    logging.info(f"[EXECUTOR] Пользователь {telegram_id} подтвердил завершение заказа {order_id}.")
    ...
    if not user:
        logging.error(f"[EXECUTOR] Ошибка: Telegram ID {telegram_id} не найден в базе users.")
        await query.answer("❌ Ошибка: ваш аккаунт не зарегистрирован.", show_alert=True)
        return
    ...
    if not order:
        logging.error(f"[EXECUTOR] Ошибка: заказ {order_id} не найден в БД.")
        await query.answer("❌ Ошибка: заказ не найден.", show_alert=True)
        return

    # 2️⃣ Ошибка: заказ уже закрыт
    if order["status"] == "Завершен":
        logging.warning(f"[EXECUTOR] Попытка повторного закрытия заказа {order_id}.")
    ...
    if order["installer_id"] != user_id:
        logging.error(f"[EXECUTOR] Ошибка: пользователь {user_id} (TG: {telegram_id}) пытается закрыть чужой заказ {order_id}.")
    ...
    try:
        ...
        logging.info(f"✅ Заказ {order_id} успешно закрыт исполнителем {user_id} (TG: {telegram_id}) в {completed_at}.")
        ...
        await update_user_state(telegram_id, "executor_idle")
...
@check_state(required_state="executor_idle")
async def handle_executor_cancel_complete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Обрабатывает отмену завершения заказа исполнителем.
    """
    query = update.callback_query
    callback_data = query.data
    order_id = int(callback_data.split("_")[-1])
    ...
    logging.info(f"[EXECUTOR] Пользователь {user_id} отменил закрытие заказа {order_id}.")
